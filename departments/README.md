---
id: departments-readme
layer: charter
status: active
owner: founder-office
unit: company-os
risk: medium
visibility: public
motion_type: none
created_at: 2026-04-03
updated_at: 2026-04-03
---

# Departments

This directory defines the permanent department structure of the Company OS.

## Design intent

- cover the major functional surfaces needed to run a constitutional workshop-company
- separate durable capability from temporary project staffing
- make delegation explicit, reviewable, and stoppable

Start with [registry.md](C:\agent\codex\company-os\departments\registry.md) and [delegation-matrix.md](C:\agent\codex\company-os\departments\delegation-matrix.md).
