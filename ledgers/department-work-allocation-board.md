...
| assembly-secretariat | run the ordinary sitting loop and maintain rollout watch across active motions | sixty-fourth sitting packet, meeting record updates, and triple-active-lane watch note | active | all active owners |
...
| portfolio-strategy | monitor the three active external routes and keep `FormPilot` as the next bounded candidate under preparation only | `oem` / `shift` / `jouzou` sequencing note and `FormPilot` preparation recommendation | active | founder-office / PMO |
| project-management-office | watch the active `oem`, `shift`, and `jouzou` lanes under the adopted control posture and support the `FormPilot` preparation-only path | triple-lane watch note and fourth-candidate preparation package | active | assembly-secretariat / constitutional-review-committee |
...
| organizational-scaling | formalize department -> division -> section growth and keep the delegation map current | subdivision registry and delegation register upkeep | active | organizational-development-learning / data-knowledge |
